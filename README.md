...
```
scripts/crawler/
├── index.js              수집 → 이월 → 중복 제거 → 마감 공고 제외 → JSON 저장
├── lib/http.js           재시도·백오프·타임아웃·쿠키·동시 요청 제어
├── lib/parse.js          HTML → 텍스트, 날짜 정규화
├── lib/taxonomy.js       지역/지원분야 통합 분류 사전
├── lib/districts.js      시·군·구 → 광역시도 사전
├── lib/projects.js       내 프로젝트별 적합도 판정
└── sources/
    ├── kstartup.js
    └── bizinfo.js
```
...
**06:35 KST**(백업 10:35 KST)에 새로 수집하고 프로덕션에 배포합니다.

> ⚠️ GitHub 의 예약 실행은 보장된 시각이 아닙니다. 실측 결과 28분~8시간까지 밀렸고
> 부하가 크면 건너뛰기도 합니다. 그래서 혼잡한 정시(:00)를 피하고 오전에 백업 실행을 하나 더 뒀습니다.
> **"몇 시 정각"이 아니라 "오전 중 갱신"으로 이해하시는 게 맞습니다.**

동작 순서: `npm ci` → `npm run crawl` → **수집 결과 검증** → `vercel deploy --prod`

#### 한 소스가 실패해도 배포는 계속됩니다

포털 쪽이 간헐적으로 응답하지 않는 일이 있습니다(실제로 GitHub 러너에서 기업마당 수집이
세 차례 실패했습니다). 이때 그날 배포 전체를 날리지 않도록:

1. **재시도** — 요청당 최대 4회, 지수 백오프. 세션 쿠키를 유지하고 페이지 사이에 간격을 둡니다.
2. **이월** — 어떤 소스의 수집량이 직전 배포의 절반에 못 미치면, 직전 데이터에서 그 소스의
   공고를 가져와 채웁니다. 마감된 공고는 이월 과정에서도 걸러집니다.
3. **검증** — 아래 셋 중 하나면 배포를 중단합니다.
   - 전체 수집량 500건 미만
   - 어떤 소스가 0건인데 이월할 데이터도 없음
   - 같은 소스가 **3일 넘게** 갱신되지 않음 (이월이 반복되면 최초 수집일을 물려주므로 감지됩니다)

수집 상태는 `programs.json` 의 `sourceHealth` 와 Actions 실행 요약에 기록됩니다.
